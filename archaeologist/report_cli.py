#!/usr/bin/env python3
"""deadcode-report — HTML report generator."""
import json
import os
import sys
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .stat-n.red {{ color: #ff6b6b; }} .stat-n.yellow {{ color: #ffd93d; }} .stat-n.blue {{ color: #74b9ff; }}
  ...
  .bar-fill.high {{ background: #e74c3c; }} .bar-fill.mid {{ background: #f39c12; }} .bar-fill.low {{ background: #3498db; }}
  ...
  .badge.safe {{ background: #d4edda; color: #155724; }} .badge.review {{ background: #fff3cd; color: #856404; }} .badge.runtime {{ background: #cce5ff; color: #004085; }} .badge.lang {{ background: #f0f0f0; color: #555; }}
...
<body>
<header>
  <div><h1>☠ Dead Code Archaeologist</h1><div style="font-size:12px;opacity:0.5;margin-top:4px">{project}</div></div>
  <div class="stats">
...
<div class="controls">
  ...
  <select class="lang-filter" onchange="setLang(this.value)">{lang_options}</select>
  <input class="search" type="text" placeholder="Search function or file..." oninput="setSearch(this.value)">
</div>
<div class="table-wrap">
<table id="results-table">
  <thead><tr><th>Confidence</th><th>Function</th><th>File</th><th>Language</th><th>Verdict</th><th>Why</th></tr></thead>
...
let currentFilter='all',currentLang='all',currentSearch='';
function barClass(s){{return s>=80?'high':s>=50?'mid':'low';}}
function verdictKey(l){{return l==='Safe to delete'?'safe':l==='Review first'?'review':'runtime';}}
function badgeClass(l){{return l==='Safe to delete'?'safe':l==='Review first'?'review':'runtime';}}
function render(){{
  const filtered=rows.filter(r=>{{
    ...
    if(currentSearch){{const q=currentSearch.toLowerCase();if(!r.name.toLowerCase().includes(q)&&!r.file.toLowerCase().includes(q))return false;}}
    return true;
  }});
  document.getElementById('tbody').innerHTML=filtered.map(r=>`<tr><td><div class="conf-bar"><div class="bar-track"><div class="bar-fill ${{barClass(r.confidence)}}" style="width:${{r.confidence}}%"></div></div><span class="conf-num">${{r.confidence}}%</span></div></td><td><div class="fn-name">${{r.name}}</div></td><td><div class="file-path" title="${{r.file}}">${{r.file}}:${{r.line}}</div></td><td><span class="badge lang">${{r.language}}</span></td><td><span class="badge ${{badgeClass(r.label)}}">${{r.label}}</span></td><td><div class="reasons">${{r.reasons.join(' · ')}}</div></td></tr>`).join('');
...
@click.option("--output", default="dead_code_report.html", help="Output HTML file path")
@click.option("--no-git", is_flag=True, help="Skip git analysis")
def cli(path, output, no_git):
    """
    Generate an interactive HTML report of dead code.

    Examples:

      deadcode-report .

      deadcode-report ./src --output ~/Desktop/report.html

    After running, open the report with:

      open dead_code_report.html       (Mac)
      start dead_code_report.html      (Windows)
      xdg-open dead_code_report.html   (Linux)
    """
    ...
    console.print(f"[dim]Found {total_funcs} functions across {len(scan_result.calls)} files[/dim]")

    git_info_map = {}
    if not no_git:
        console.print("[dim]Analyzing git history...[/dim]")
    ...
    lang_options = '<option value="all">All languages</option>' + "".join(
        f'<option value="{l}">{l.capitalize()}</option>' for l in langs
    )
    ...
    for c in candidates:
        ...
        rows.append({
            "name": c.name, "file": rel_file, "line": c.line,
    ...
    html = HTML_TEMPLATE.format(
        project=abs_path,
        safe=len(safe), review=len(review), runtime=len(runtime),
        total=total_funcs, all_count=len(candidates),
    ...
    console.print(f"\n[green]✓ Report saved:[/green] {abs_output}")
    console.print(f"[dim]  Open with: open {abs_output}[/dim]\n")
